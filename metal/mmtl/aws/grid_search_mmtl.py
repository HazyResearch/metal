...
def create_command_dict(config_path):
    ...
    COMMAND = "python metal/mmtl/launch.py --tasks QNLI --n-epochs 1 --log-every 0.25 --score-every 0.25 --max-len 256 --batch-size 1 --checkpoint-dir ./checkpoint --checkpoint-metric QNLI/valid/accuracy --checkpoint-metric-mode max --max-datapoints 16 --override-train-config ../config --device -1"
    return {
