                    ...
                    for i, (split, data_loader) in enumerate(data_loaders.items()):
                        ...
                        if args.eval_split == "dev":
                            ...
                            if (task.name, state) in predictions:
                                print("Ensembled:")
                                for i in range(len(Ys_probs[task.name])):
                                    Ys_probs[task.name][i] = (
                                        Ys_probs[task.name][i]
                                        + predictions[(task.name, state)][i]
                                    ) / 2.0
                                    column = np.argmax(Ys_probs[task.name][i])
                                    Ys_preds[task.name][i] = column + 1
                                task_metrics_dict = scorer.score(
                                    Ys[task.name],
                                    Ys_probs[task.name],
                                    Ys_preds[task.name],
                                ...
                                predictions[(task.name, state)] = Ys_probs[task.name]
                        if args.eval_split == "test":
                            if (task.name, state) in predictions:
