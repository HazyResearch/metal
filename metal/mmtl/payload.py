...
class Payload(object):
    ...
    def add_label_set(self, task_name, label_list=None, label_fn=None, verbose=True):
    ...
    def remove_label_set(self, task_name, verbose=True):
        self.data_loader.dataset.labels.pop(task_name)
        self.task_names.remove(task_name)

        if verbose:
            print(f"Removed label_set for task {task_name} from payload {self.name}.")
