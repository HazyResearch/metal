...
def eval_model(model, data, eval_dict):
    ...
    for slice_name, eval_idx in eval_dict.items():
        slice_scores[slice_name] =             model.score_on_slice(data, eval_idx, metric="accuracy", verbose=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--variable", choices=["op", "acc", "cov"], 
                        help="variable we are varying in simulation")
    parser.add_argument("--save-dir", type=str, 
                        help="where to save results")
    parser.add_argument("--n", type=int, default=25,
                        help="num trials to run of simulation")
    parser.add_argument("--x-range", type=float, nargs="+", default=None,
