...
class SnorkelDataset(Dataset):
    ...
    def __init__(
        ...
        if self.selected_idx is not None:
            self.X = [
                x for idx, x in enumerate(self.X) if idx in self.selected_idx
            ]
            self.Y = torch.tensor(
                [y for idx, y in enumerate(self.Y) if idx in self.selected_idx]
            )
            if self.L is not None:
                self.Y = torch.tensor(
                    [l for idx, l in enumerate(self.L) if idx in selected_idx]
                )
